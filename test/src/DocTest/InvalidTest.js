...
describe('test invalid code', ()=>{
  it('has invalid code log.', ()=>{
    ...
    assert(InvalidCodeLogger._logs[0].filePath.includes('test/fixture/package/src/Invalid/CodeSyntax.js'));
    assert.deepEqual(InvalidCodeLogger._logs[0].log, [1, 2]);

    assert(InvalidCodeLogger._logs[1].filePath.includes('test/fixture/package/src/Invalid/DocSyntax.js'));
