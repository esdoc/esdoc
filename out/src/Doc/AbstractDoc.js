...
var _ASTNodeContainer = _interopRequireDefault(require("../Util/ASTNodeContainer.js"));
...
class AbstractDoc {
  ...
  constructor(ast, node, pathResolver, commentTags = []) {
    ...
    this._value = {};
    Reflect.defineProperty(this._node, 'doc', {
      value: this
    });
    this._value.__docId__ = _ASTNodeContainer.default.addNode(node);
  ...
  _$param() {
    ...
    for (const value of values) {
      const {
        ...
        paramName,
        paramDesc
      } = _ParamParser.default.parseParamValue(value);

      if (!typeText || !paramName) {
        _InvalidCodeLogger.default.show(this._pathResolver.fileFullPath, this._node);
  ...
  _$return() {
    ...
    const {
      ...
      paramName,
      paramDesc
    } = _ParamParser.default.parseParamValue(value, true, false, true);
  ...
  _$type() {
    ...
    const {
      ...
      paramName,
      paramDesc
    } = _ParamParser.default.parseParamValue(value, true, false, false);
  ...
  _$decorator() {
    ...
    for (const decorator of this._node.decorators) {
      ...
      switch (decorator.expression.type) {
        ...
        case 'CallExpression':
          value.name = (0, _generator.default)(decorator.expression).code.replace(/[(][\S\s.]*/, '');
          value.arguments = (0, _generator.default)(decorator.expression).code.replace(/^[^(]+/, '');
          break;
  ...
  _resolveLongname(name) {
    let importPath = _ASTUtil.default.findPathInImportDeclaration(this._ast, name);

    if (!importPath) return name;

    if (importPath.charAt(0) === '.' || importPath.charAt(0) === '/') {
      if (!_path.default.extname(importPath)) importPath += '.js';
  ...
  _findClassLongname(className) {
    ...
    const importPath = _ASTUtil.default.findPathInImportDeclaration(this._ast, className);

    if (importPath) return this._resolveLongname(className); // find in external
