...
class DocFactory {
  ...
  constructor(ast, pathResolver) {
    ...
    this._inspectExportNamedDeclaration(); // file doc


    const doc = new _FileDoc.default(ast, ast, pathResolver, []);

    this._results.push(doc.value); // ast does not child, so only comment.
  ...
  _inspectExportDefaultDeclaration() {
    ...
    for (const exportNode of this._ast.program.body) {
      ...
      switch (exportNode.declaration.type) {
        ...
        case 'Identifier':
          {
            const varNode = _ASTUtil.default.findVariableDeclarationAndNewExpressionNode(exportNode.declaration.name, this._ast);
        ...
        default:
          _colorLogger.default.w(`unknown export declaration type. type = "${exportNode.declaration.type}"`);
      ...
      const functionNode = _ASTUtil.default.findFunctionDeclarationNode(exportNode.declaration.name, this._ast);
      ...
      const variableNode = _ASTUtil.default.findVariableDeclarationNode(exportNode.declaration.name, this._ast);
  ...
  _inspectExportNamedDeclaration() {
    ...
    for (const exportNode of this._ast.program.body) {
      ...
      for (const specifier of exportNode.specifiers) {
        ...
        const varNode = _ASTUtil.default.findVariableDeclarationAndNewExpressionNode(specifier.exported.name, this._ast);
        ...
        const functionNode = _ASTUtil.default.findFunctionDeclarationNode(specifier.exported.name, this._ast);
        ...
        const variableNode = _ASTUtil.default.findVariableDeclarationNode(specifier.exported.name, this._ast);
  ...
  push(node, parentNode) {
    ...
    this._results.push(...results); // for trailing comments.
  ...
  _traverseComments(parentNode, node, comments) {
    ...
    for (const comment of comments) {
      const tags = _CommentParser.default.parse(comment);
  ...
  _decideMethodDefinitionType(node) {
    ...
    if (this._processedClassNodes.includes(classNode)) {
      ...
      _colorLogger.default.w('this method is not in class', node);
  ...
  _decideClassPropertyType(node) {
    ...
    if (this._processedClassNodes.includes(classNode)) {
      ...
      _colorLogger.default.w('this class property is not in class', node);
    ...
    switch (node.right.type) {
      ...
      default:
        if (node.left.type === 'MemberExpression' && node.left.object.type === 'ThisExpression') {
          ...
          if (!this._processedClassNodes.includes(classNode)) {
            _colorLogger.default.w('this member is not in class.', this._pathResolver.filePath, node);
