  ...
  _createClass(DocResolver, [{
    ...
    value: function _resolveExtendsChain() {
      ...
      var extendsChain = function extendsChain(doc) {
        ...
        if (chains.length) {
          // direct subclass
          var _superClassDoc = _this2._builder._findByName(chains[0])[0];
          if (_superClassDoc) {
            if (!_superClassDoc._custom_direct_subclasses) _superClassDoc._custom_direct_subclasses = [];
            _superClassDoc._custom_direct_subclasses.push(selfDoc.longname);
          }
          ...
          try {
            for (var _iterator6 = chains.slice(1)[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
              ...
              _superClassDoc = _this2._builder._findByName(superClassLongname)[0];
              if (_superClassDoc) {
                if (!_superClassDoc._custom_indirect_subclasses) _superClassDoc._custom_indirect_subclasses = [];
                _superClassDoc._custom_indirect_subclasses.push(selfDoc.longname);
              }
            }
          ...
          try {
            for (var _iterator7 = chains[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
              var _superClassLongname = _step7.value;

              _superClassDoc = _this2._builder._findByName(_superClassLongname)[0];
              if (!_superClassDoc) continue;

              // indirect implements
              if (_superClassDoc.implements) {
                var _selfDoc$_custom_indi;

                if (!selfDoc._custom_indirect_implements) selfDoc._custom_indirect_implements = [];
                (_selfDoc$_custom_indi = selfDoc._custom_indirect_implements).push.apply(_selfDoc$_custom_indi, _toConsumableArray(_superClassDoc.implements));
              }
      ...
      var implemented = function implemented(doc) {
        ...
        try {
          for (var _iterator9 = (selfDoc._custom_indirect_implements || [])[Symbol.iterator](), _step9; !(_iteratorNormalCompletion9 = (_step9 = _iterator9.next()).done); _iteratorNormalCompletion9 = true) {
            var _superClassLongname2 = _step9.value;

            var _superClassDoc2 = _this2._builder._findByName(_superClassLongname2)[0];
            if (!_superClassDoc2) continue;
            if (!_superClassDoc2._custom_indirect_implemented) _superClassDoc2._custom_indirect_implemented = [];
            _superClassDoc2._custom_indirect_implemented.push(selfDoc.longname);
          }
        } catch (err) {
    ...
    value: function _resolveTestRelation() {
      ...
      try {
        for (var _iterator13 = testDocs[Symbol.iterator](), _step13; !(_iteratorNormalCompletion13 = (_step13 = _iterator13.next()).done); _iteratorNormalCompletion13 = true) {
          var _testDoc = _step13.value;

          var desc = [];
          var parents = (_testDoc.memberof.split('~')[1] || '').split('.');
          ...
          try {
            for (var _iterator15 = parents[Symbol.iterator](), _step15; !(_iteratorNormalCompletion15 = (_step15 = _iterator15.next()).done); _iteratorNormalCompletion15 = true) {
              var parent = _step15.value;

              var _doc = this._builder._find({ kind: ['testDescribe', 'testIt'], name: parent })[0];
              if (!_doc) continue;
              desc.push(_doc.descriptionRaw);
            }
          } catch (err) {
          ...
          desc.push(_testDoc.descriptionRaw);
          _testDoc.testFullDescription = desc.join(' ');
        }
      } catch (err) {
